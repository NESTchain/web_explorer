    ...
    function networkService($http, appConfig, utilities) {
        ...
        return {
            ...
            getBigBlocks: function(callback) {
                $http.get(appConfig.urls.elasticsearch_wrapper + "/get_account_history2?from_date=now-1w&to_date=now&type=aggs&agg_field=block_data.block_num&size=20")
            ...
            getBigTransactions: function(callback) {
                $http.get(appConfig.urls.elasticsearch_wrapper + "/get_account_history2?from_date=now-1d&to_date=now&type=aggs&agg_field=block_data.trx_id.keyword&size=20")
                    .then(function (response) {
